// 只允许本任务飞牛隔离库；不运行 Seed，不删除用户，不写业务库。
...
const personalTables = ['users', 'user_roles', 'roles', 'role_permissions', 'permissions', 'community_profiles', 'refresh_tokens', 'lesson_progress', 'learning_notes', 'community_follows', 'community_bookmarks', 'resource_watch_progress', 'activity_events']
async function personal() {
  ...
  for (const { tablename } of tables.filter(t => personalTables.includes(t.tablename) || /mfa|verification|session|identity|credential|collection.*reference/.test(t.tablename))) {
    ...
    result[tablename] = await prisma.$queryRawUnsafe('SELECT md5(coalesce(string_agg(row_hash,\'\' ORDER BY row_hash),\'\')) AS hash, count(*)::int AS count FROM (SELECT md5(to_jsonb(t)::text) row_hash FROM "' + tablename + '" t) s')
  }
  return result
}
async function contentCounts() {
  return { files: await prisma.fileRecord.count(), posts: await prisma.communityPost.count(), replies: await prisma.communityComment.count(), courses: await prisma.course.count(), versions: await prisma.courseVersion.count(), audits: await prisma.auditLog.count(), registry: await prisma.systemSetting.findMany({ where: { key: { startsWith: 'project_content:' } }, orderBy: { key: 'asc' } }) }
}
async function main() {
  ...
  assert.deepEqual(await personal(), before); await assertContentReady(prisma); mark('首次同步完成且全部账号、凭据、权限和个人记录不变')
  ...
  if (mode === 'fresh') { assert.equal(first.counts.created + first.counts.updated, 356); assert.equal(courses.length, 24); assert.equal(await prisma.user.count(), 1); assert.equal(await prisma.communityPost.count({ where: { sourceType: 'project_content' } }), 124); assert.equal(await prisma.communityComment.count({ where: { sourceType: 'project_content' } }), 200) }
  mark('完整课程课时与三类内容数量')
  ...
  try {
    const updated = structuredClone(bundle); updated.release++; updated.versions.community++; updated.versions.courses++
