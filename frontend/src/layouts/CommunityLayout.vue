...
<template>
  <div class="community-shell" :class="{ 'sidebar-collapsed': collapsed, 'community-wide': wide }">
    ...
    <header class="community-mobile-header"><RouterLink v-if="route.name === 'course'" class="back-button" to="/topics" aria-label="返回通识课程列表" title="返回通识课程列表"><AppIcon name="back" /></RouterLink><RouterLink class="brand" to="/community"><span class="brand-mark">{{ BRAND_MARK }}</span><strong>{{ BRAND_NAME }}</strong></RouterLink><button class="icon-button" aria-label="更多功能" @click="menuOpen = true"><AppIcon name="menu" /></button></header>
