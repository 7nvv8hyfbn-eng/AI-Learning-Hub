...
const load = async () => {
  ...
  try {
    ...
    store.syncAuthors([loaded.author, ...page.items.map(comment => comment.author)])
...
const loadComments = async (parentId?: string) => {
  ...
  try {
    ...
    if (epoch !== loadEpoch) return
    store.syncAuthors(page.items.map(comment => comment.author))
...
<template><section><header v-if="!discussionOnly" class="community-page-heading"><RouterLink :to="store.lastFeedLocation" class="back-button" aria-label="返回社区" title="返回社区"><AppIcon name="back" /></RouterLink><h1>学习讨论</h1></header><p v-if="error" class="community-error" role="alert">{{ error }} <button @click="load">重试</button></p><CommunityPostCard v-if="post && !discussionOnly" :post="post" detail @changed="load" @hidden="post = null" />
