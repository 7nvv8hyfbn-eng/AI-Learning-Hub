...
<template><section><header v-if="!discussionOnly" class="community-page-heading"><RouterLink :to="store.lastFeedLocation"><AppIcon name="back" :size="15" />返回社区</RouterLink><h1>学习讨论</h1></header><p v-if="error" class="community-error" role="alert">{{ error }} <button @click="load">重试</button></p><CommunityPostCard v-if="post && !discussionOnly" :post="post" detail @changed="load" @hidden="post = null" />
