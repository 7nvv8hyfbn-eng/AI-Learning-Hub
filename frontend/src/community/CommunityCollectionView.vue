...
<template><section><header class="community-page-heading"><div><RouterLink to="/community" class="back-button" aria-label="返回社区发现" title="返回社区发现"><AppIcon name="back" /></RouterLink><h1>{{ title }}</h1><p v-if="topic">{{ topic.description }}</p></div><FollowButton v-if="topic" :active="topic.following" :pending="store.operations[`follow:topic:${topic.id}`]" @click="follow" /></header>
