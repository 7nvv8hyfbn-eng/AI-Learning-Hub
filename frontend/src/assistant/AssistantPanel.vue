...
import { assistantState as state, closeAssistant, sendAssistantQuestion } from './assistantState'
const draft = ref(''), input = ref<HTMLTextAreaElement>(), messages = ref<HTMLElement>()
...
<template>
  <section id="assistant-panel" class="assistant-panel" :class="`is-${state.config.position}`" role="dialog" aria-labelledby="assistant-title" @keydown.esc.stop.prevent="closeAssistant">
    ...
    <div ref="messages" class="assistant-messages" aria-live="polite" :aria-busy="state.busy">
    ...
    <form @submit.prevent="send"><label for="assistant-input">向{{ state.config.name }}提问</label><div class="assistant-compose"><textarea id="assistant-input" ref="input" v-model="draft" rows="2" maxlength="2000" placeholder="输入你的问题…" @keydown="keydown" /><button class="button primary" type="submit" :disabled="state.busy || !draft.trim()">{{ state.busy ? '回答中' : '发送' }}</button></div><small>Enter 发送 · Shift+Enter 换行 · AI 回答请结合资料核对</small></form>
  </section>
</template>
<style scoped>
