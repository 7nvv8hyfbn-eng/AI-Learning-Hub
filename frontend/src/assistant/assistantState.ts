...
import { assistantConfigDefaults, type AssistantMessageDto } from '@ai-learning-hub/contracts'
...
export function closeAssistant() { cancelAssistantRequest(); assistantState.open = false }
...
export async function sendAssistantQuestion(raw: string, retryId?: number) {
  ...
  if (!question || question.length > 2000 || assistantState.busy || !assistantState.ready || !assistantState.config.enabled) return
