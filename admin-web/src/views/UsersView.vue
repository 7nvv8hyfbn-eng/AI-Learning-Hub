...
import UserPublicBadges from '../components/UserPublicBadges.vue'
...
<template>
  ...
  <section v-if="selected" class="panel users-detail">
    ...
    <UserPublicBadges v-if="can('user.badge.manage')" :key="`badges:${selected.user.id}`" :user-id="selected.user.id" :user-revision="selected.user.revision" :username="selected.user.username" :expertise-topics="selected.community.expertiseTopics" :permissions="session.user?.permissions" :readonly="selected.user.id === session.user?.id" @saved="inspect(selected!.user.id)" />
    <FrontendModeratorGrants v-if="can('user.moderator.manage') && selected.user.id !== session.user?.id" id="frontend-moderator-grants" :key="selected.user.id" :detail="selected" @saved="inspect(selected!.user.id)" />
